...
CREATE TABLE IF NOT EXISTS receipts (
    ...
    -- Атрибуты партии/строки поступления (справочно, из исходного Excel
    -- либо вносимые вручную): серийный номер по строке партии (может не
    -- совпадать 1:1 с unit_id, если серийная единица ещё не заведена),
    -- дата производства партии, курс валюты на момент покупки, общая
    -- стоимость партии в CNY. Таможенной пошлины здесь намеренно НЕТ —
    -- она привязана к позиции (parts.customs_duty_percent), а не к партии,
    -- см. комментарий у этого поля в таблице parts выше.
