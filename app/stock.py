...
GUARANTEED_RESOURCE_REASONS = ("damage",)
...
def guaranteed_resource_hours(part_id):
    ...
    момент списания среди списаний ЭТОЙ детали строго с причиной
    "повреждение" (GUARANTEED_RESOURCE_REASONS = 'damage') — то есть реально
    зафиксированный случай повреждения/износа при эксплуатации. Причины
    "ремонт" (компонент мог быть заменён планово, не обязательно
    выработав ресурс), "брак" ('defect', заводской дефект, не связан с
    реальной эксплуатацией) и "другое" ('other', слишком неопределённая
    категория) в расчёт намеренно не входят — они дали бы заниженную и
    недостоверную оценку ресурса. Учитывается и списание прямо со страницы
    инструмента (кнопка «Списать» рядом со «Снять» у установленного
    компонента) — оно проходит через тот же write_offs с тем же набором
    причин. Наработка серийной единицы после списания больше не меняется
    (единица снята с эксплуатации), поэтому circulation_hours на этот
    момент и есть её "стаж" до выбытия. Возвращает None, если по этой
    детали ещё не было ни одного такого списания серийной единицы (для
    несерийных расходников понятие "гарантированного ресурса" неприменимо —
    там нет units.circulation_hours)."""
