...
def written_off():
    ...
    см. app/stock.py: GUARANTEED_RESOURCE_REASONS). Плановая замена
    ('repair'), заводской брак ('defect') и прочие причины ('other') сюда
    не попадают — это не подтверждённый случай реальной выработки ресурса
    при эксплуатации."""
    q = request.args.get("q", "").strip()
    tool_size = request.args.get("tool_size", "").strip()
